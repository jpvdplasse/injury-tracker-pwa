...
export default function InjuryDetail({ injury, onUpdateStatus, onUpdate, onDelete, onClose }: InjuryDetailProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-lg bg-surface-800 rounded-t-2xl animate-slide-up max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="px-5 pb-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full bg-surface-600 flex items-center justify-center text-gray-400 hover:text-white"
            >
              ✕
            </button>
          </div>
          ...
          {injury.notes && (
            <div className="bg-surface-700 rounded-xl p-4 mb-5">
              ...
              <p className="text-sm text-gray-200">{injury.notes}</p>
            </div>
          )}

          {/* Status Progress */}
          <div className="mb-5">
            <div className="text-sm text-gray-400 mb-3">Herstel Voortgang</div>
          ...
          {/* Recovery Notes */}
          <div className="mb-5">
            <div className="text-sm text-gray-400 mb-2">Herstel Notities</div>
            <textarea
              value={recoveryNotes}
          ...
          {/* Recovery Date */}
          {injury.recoveryDate && (
            <div className="bg-status-healed/10 border border-status-healed/20 rounded-xl p-4 mb-5">
              <div className="text-sm text-status-healed font-medium">
          ...
          {!showDeleteConfirm ? (
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 bg-red-500/20 text-red-400 rounded-xl text-sm font-medium hover:bg-red-500/30 transition-colors"
              >
                Bevestig Verwijderen
              </button>
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 py-3 bg-surface-700 text-gray-400 rounded-xl text-sm font-medium hover:text-white transition-colors"
              >
                Annuleren
              </button>
            </div>
